...
#pragma fragment frag
	...
	v2g vert(appdata_full v)
	{
		...
		float phase0 = (_WaveHeight)* sin((_Time[1] * _WaveSpeed) + (v0.x * _WaveLenght) + (v0.z * _WaveLenght) + rand2(v0.xzz));
	...
	void geom(triangle v2g IN[3], inout TriangleStream<g2f> triStream)
	{
		...
		float ambientLighting = UNITY_LIGHTMODEL_AMBIENT.rgb * _Color.rgba;
		float diffuseReflection = attenuation * _LightColor0.rgb * _Color.rgba * max(0.0, dot(normalDirection, lightDirection));
	...
	{
		return float4(IN.specularColor + IN.diffuseColor, _Alpha);
	}
